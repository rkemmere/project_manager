...
// ';
    ...
    $sql = 'SELECT * FROM (
                          SELECT id, name, domain, is_ssl, status FROM `rex_project_manager_domain` ORDER BY domain ASC
                          ) AS D
...
//     $list->setColumnParams($thIcon, ['func' => 'edit', 'data_id' => '###id###']);    
    ...
    $list->addColumn($this->i18n('pm_client_version'), false, -1, ['<th>###VALUE###</th>', '<td class="rex-table-cms-version">###VALUE###</td>']);
    ...
    $list->addColumn(rex_i18n::msg('view'), '<i class="rex-icon rex-icon-view"></i> ' . rex_i18n::msg('view'));
